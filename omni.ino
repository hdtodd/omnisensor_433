...
// For Pico-2
// VSYS/3.0 is on GPIO29 in Pico 2
#define VSYSPin  29
#define ADCRES   12
#define RES      ((float)(1<<ADCRES))

#define DELAY 29365   // Time between messages in ms, less 635ms for overhead on Pico 2
//#define DELAY 4365 // Time between messages in ms, less 635ms for overhead on Pico 2
...
class omni : public ISM_Device {
  ...
  public:
    ...
    */
    ...
    {
      if ( (type == 0x00) || (id == 0x00) ) {
	  DBG_println(F("Can't create message with type=0 or id=0"));
	  for (uint8_t j=0; j<9; j++) msg[j]=0x00;
            ...
            uint8_t &iHum, uint8_t &oHum, uint16_t &press, uint8_t &volts)
    {
	  ...
	  DBG_println(F("Attempt unpack of invalid message packet: CRC8 checksum error"));
            type  = 0;
            id    = 0;
            iTemp = 0;
            oTemp = 0;
...
void loop(void)
{
    ...
    ihumf  = 34.8;  //%
    ohumf  = 49.3;  //%
    pressf = 9993.0;//Pa (not hPa; BME reports Pa)
    voltsf = 4.936; //volts
    ...
    // Reading was in Pa; we want hPa; so /10.0 to hPa, then *10 to scale for transmission
    press = (uint16_t)(pressf + 0.5);                                   // round with the 0.5
    volts = (uint8_t)(100.0 * (voltsf - 3.00) + 0.5);

    // Unpack BME readings, repack for ISM transmission, and create the waveform
    // Use Pico 2 core temp as "outside" temp, otemp
    itemp  = (uint16_t) ((bme.temperature + 0.05 + BME_TEMP_OFFSET) * 10); // adjust & round
    ...
    volts =  (uint8_t)  ( 100.0 * ( ( (float) analogRead(VSYSPin) )/ RES * 3.0 * 3.3 - 3.0 )+ 0.5 );

    // Pack the message and create the waveform
    ...
    om.unpack_msg(msg, type, id, itemp, otemp,
            ihum, ohum, press, volts);
